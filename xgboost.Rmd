...
param_xg <- makeParamSet(
                        ...
                        #  makeDiscreteParam("eta", values = seq(0.1, 0.2)),
                          ...
                          makeDiscreteParam("subsample", values = c(0.8, 0.9, 1))
                        )


ctrl = makeTuneControlGrid()

res <- makeResampleDesc(stratify = TRUE, method = "CV", predict = "both")

sd_auc <- setAggregation(auc, test.sd)
train_acc <- setAggregation(acc, train.mean)
train_auc <- setAggregation(auc, train.mean)
...
tune_rpart <-tuneParams(learner = class_rpart_german,
                        task = task_german,
                        resampling = res,
                        measures = list(acc, auc, sd_auc, fnr, fpr, f1, train_acc, train_auc),
                        par.set =  param_rpart,
                        control = ctrl,
                        show.info = TRUE
                        )
...
tune_xg <- tuneParams(learner = class_xgboost_german,
                        task = task_german,
                        resampling = res,
                        measures = list(acc, train_acc, auc, train_auc, sd_auc, fnr, fpr, f1),
                        par.set =  param_xg,
                        control = ctrl,
                        show.info = TRUE
                        )
...
g + geom_boxplot(aes(x= nrounds, y = auc.test.mean))
